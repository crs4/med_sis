...
class S4MMetadataService:
    ...
    def initialize_metadata(self, dataset_id):
        try:
            files=[('metadata_file',('metadata.xml',open(f"/usr/src/metadata/{dataset_id}.xml",'rb'),'text/xml'))]
            headers = self.auth_header
            endpoint = f"{self.base_url}/api/v2/datasets/{dataset_id}/metadata"
            response = requests.request("PUT", endpoint, headers=headers, data={}, files=files)
            endpoint = f"{self.base_url}/api/v2/resources/{dataset_id}/permissions"
            payload = json.dumps({
                "users": [
                    { "id": 1001, "permissions": "manage" },
                    { "id": 1002, "permissions": "manage" }
                ],
                "groups": [
                    { "id": 2, "permissions": "view" },
                    { "id": 3, "permissions": "download" }
                ]
            })
            response = requests.request("PUT", endpoint, headers=headers, data={}, files=files)
            return 'ok'
