...
# -----------------------------------------------------------------------------
def main():
    # Parameters: the train needs two file !!!!
    training_set_files =  ["/usr/src/s4m_catalogue/hydro_ptf/HYDRO_VOL_oct25.xlsx",
                           "/usr/src/s4m_catalogue/hydro_ptf/HYDRO_GRAV_oct25.xlsx"]
    n_iterations = 100
    test_size = 0.3
    random_state = 42
    pot_values = [4, 10, 33, 100, 200, 300, 1500]
    work_dir = '/usr/src/s4m_catalogue/hydro_ptf/'
    ## two training set 'VOL' with Bulk Density and 'GRAV' without Bulk density 
    current = 'VOL'
    ############### TRAINING WITH HYDRO_VOL_....
    for fname in training_set_files: 
        data = pd.read_excel(fname, sheet_name="Foglio1")
        ...
        with pd.ExcelWriter(os.path.join(work_dir, current + 'monte_carlo_performance_metrics.xlsx')) as writer:
        ...
        detailed_df.to_csv(os.path.join(work_dir, current + 'detailed_predictions.csv'), index=False)
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(work_dir, current + '01_Performance_Metrics_Distribution.png'))
        plt.close()
        ...
        ax.set_aspect('equal')
        plt.tight_layout()
        plt.savefig(os.path.join(work_dir, current + '02_MLP_Observed_vs_Predicted.png'))
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(work_dir, current + '03_MLP_Residual_Analysis.png'))
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(work_dir, current + '04_MLP_RMSE_vs_Matric_Potential.png'))
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(work_dir, current + '05_MLP_Performance_by_Dataset.png'))
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(work_dir, current + '06_MLP_Feature_Importance.png'))
        plt.close()
        ...
        ax.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.savefig(os.path.join(work_dir, current + '07_MLP_Training_Convergence.png'))
        plt.close()
        ...
        mlp_config_df.to_csv(os.path.join(work_dir, current + 'best_mlp_configuration.csv'), index=False)
        ...
        final_models_params.to_csv(os.path.join(work_dir, current + 'final_models_parameters.csv'), index=False)
        
        
        # Save final models
        with open(os.path.join(work_dir, current + 'final_mlp_model.pkl'), 'wb') as file:
            dill.dump(final_mlp, file)
        with open( os.path.join(work_dir, current + 'feature_preprocessor.pkl'), 'wb') as file:
            dill.dump(feature_preprocessor, file)
        with open( os.path.join(work_dir, current + 'pot_preprocessor.pkl'), 'wb') as file:
            dill.dump(pot_preprocessor, file)
        
        # Save model architecture information
        with open(os.path.join(work_dir, current + 'model_architecture.txt'), 'w') as f:
    ...
    print("All analysis complete! Files saved in:", work_dir)
